...
**Existing `ames` DB (tables already applied before Boot):** use profile `dev`, which sets `spring.flyway.baseline-on-migrate=true` and `baseline-version=3` so Flyway creates `flyway_schema_history` without re-running V1–V3. **V4 still runs** after baseline. Fresh empty DB: Flyway runs V1→V4 normally.
